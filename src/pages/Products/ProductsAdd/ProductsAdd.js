  ...
  async handleDataSubmit(e) {
    ...
    const prodData = await axios({
      ...
      data: {
        ...
        restaurantes:
          this.state.restaurantes.length <= 1
            ? this.state.restaurantes[0]
  ...
  async handleImageUpload(id) {
    ...
    await axios({
      ...
      url:
        process.env.NODE_ENV !== "production"
          ? `/upload`
          : `https://quiick-281820.rj.r.appspot.com/upload`,
      data,
      onUploadProgress: (progress) =>
        this.setState({
          percent: loadedPercent(progress.loaded, progress.total),
        }),
      headers: { Authorization: `Bearer ${this.state.userToken}` },
    });
  }
  ...
  render() {
    ...
    return !resetForm ? (
      <div className="product__wrapper">
        <form
          className="product__form-wrapper"
        ...
        >
          <div className="product__form">
            ...
            <div className="product__form-element">
              ...
              <select name="categorias" onChange={this.handleSelectionData}>
                {userCategories !== null
                  ? userCategories.map((category) => {
                      return (
                        <option key={category.id} name={category.categoria}>
                          {category.categoria}
                        </option>
                      );
                    })
            ...
            <div className="product__form-divided-wrapper">
              <div className="product__form-element">
            ...
            <div className="product__form-divided-wrapper">
              <div className="product__form-element">
                ...
                <select name="promo" onChange={this.handleProdData}>
              ...
              {userLocals !== null
                ? userLocals.map((local) => {
                    return (
                      <div
                        key={local.restid}
                        className={`product__form-local-selector`}
                        onClick={() => this.setLocals(local)}
                      >
                        <strong>{local.name}</strong>
                        <div
                          className={`switch__selector-wrapper ${
                            activeSelections.includes(local.restid)
                              ? "switch__selector-wrapper--active"
                              : "switch__selector-wrapper--deactive"
                          }`}
                        >
                          <div
                            className={`switch ${
            ...
            <div className="product__form-element product__form-element-full product-form-attach-file">
              {picture !== null ? (
                <div className="indicator">
                  <CloudIconSuccess />
                  <span>Imagen agregada con éxito</span>
                </div>
              ) : (
                <div className="indicator">
                  <CloudIcon />
                  <span>Arrastra la imagen o haz click aquí para cargarla</span>
                </div>
              )}
              {resetState ? (
                <Spinner />
              ...
              {picture !== null ? (
                <div className="product-form-file">
                  <div className="product-form-file-name">
                    {<Icon faIcon={faCheckCircle} />}
                    {picFileName}
                  </div>
                  <div
                    className="product-form-remove-file"
                    onClick={this.removeProdPic}
                  >
